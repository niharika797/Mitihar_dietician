...
## Schema Drift Cleanup (2026-07-13)

- **`db_models.py` fully migrated to `Mapped[]`/`mapped_column()` except `FoodItem`/`MealTemplate`** (hard-excluded, `DO NOT MODIFY` comment in file). Nullability was transcribed 1:1 from each `Column(...)`'s existing `nullable=` kwarg — verified against live schema (both local dev and staging) with zero mismatches across all 24 classes.
- **Migration `651cd3d46fa9`** dropped the dead `beverages` table (0 rows, superseded by Session 22E's `food_items(slot_type='beverage')` approach — see `meal_plan.py` `list_beverages()` docstring) and `food_items.instructions` (0 non-null rows, never read anywhere). Confirmed dead on **both** local dev and staging via a temporary read-only Cloud Run job before dropping. Downgrade round-trip tested — restores both exactly. Replaces `scripts/drop_instructions.py`, which bypassed Alembic (raw engine `ALTER TABLE`) — do not run that script, it's now redundant and its execution path was never migration-tracked.
- **3 indexes existed live but were missing from the model** (`idx_patients_doctor_id`, `idx_patient_requests_doctor_id`, `idx_sc_reserved_by`) — all deliberate Jul 1-3 performance fixes (full-table-scan avoidance on doctor-dashboard queries), added via raw migrations without a matching model `Index()`. Added to `db_models.py` now — `alembic check` is clean except for the one deliberate exception below.
- **`food_items.original_name` is 0/2137 populated on both local dev and staging as of 2026-07-13 — left in place, not dropped.** It's the rollback-safety column for `clean_dish_names.py` (still a working, re-runnable script), not dead schema like `instructions`. See the Known Pending Issues entry in the root `CLAUDE.md` for the resume-safety caveat before running that script again.
