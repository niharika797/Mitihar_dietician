...
- **`POST /progress/log/weight` is slow (Layer 3/4 watch item)**: endpoint calls `_handle_weight_change` → `DietPlanService.generate_diet_plan()` → Gemini API synchronously on every weight update. Local test hit httpx 5s default timeout during this session (Gemini latency variability — no code change caused it, `_handle_weight_change` unchanged since `5fbd380` 2026-03-19). Test now has `timeout=60`. In production (Cloud SQL + Cloud Run cold start + Gemini) 60s may still not be enough; `_handle_weight_change` should be made async/background (e.g. fire-and-forget task) before launch.
