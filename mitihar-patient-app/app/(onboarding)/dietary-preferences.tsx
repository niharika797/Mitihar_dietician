...
export default function DietaryPreferencesScreen() {
  ...
  const handleContinue = () => {
    update({ diet_type: dietType, region, meals_per_day: meals, fasting_days: fastingDays });
        ...
        <View>
          ...
          <View style={s.dietList}>
            {DIET_TYPES.map(dt => {
              ...
              return (
                <Pressable key={dt.key} onPress={() => setDietType(dt.key)} style={[s.dietCard, sel && s.dietCardSel]}>
                  <Text style={s.dietEmoji}>{dt.emoji}</Text>
                  <Text style={s.dietLabel}>{dt.key}</Text>
